...
void heapifyD(minHeap* min, int i){
    ...
    int filhoE = 2*i + 1;
    int filhoD = 2*i + 2;
    if (filhoE < min->tamanho && min->heapArray[filhoE].chave < min->heapArray[menor].chave) menor = filhoE;
    if (filhoD < min->tamanho && min->heapArray[filhoD].chave < min->heapArray[menor].chave) menor = filhoD;
    if (menor != i){
        trocar(min, i, menor);
        heapifyD(min, menor);
    }
